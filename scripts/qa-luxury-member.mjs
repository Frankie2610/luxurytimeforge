...
for(const width of sizes){
 ...
 const cta=track.locator('.tf709-member-invitation');
 assert(await cta.count()===1,'track '+width+'px: luxury Member invitation missing');
 const ctaGeometry=await cta.evaluate(el=>{
   const outer=el.getBoundingClientRect();
   const copy=el.querySelector('.tf709-invitation-copy');
   const action=el.querySelector('.tf709-invitation-action');
   const headline=copy?.querySelector('h3');
   const c=copy?.getBoundingClientRect(),a=action?.getBoundingClientRect();
   return{panel:[outer.left,outer.right,outer.top,outer.bottom],
    copy:c?[c.left,c.right,c.top,c.bottom]:null,
    action:a?[a.left,a.right,a.top,a.bottom]:null,
    foreground:headline?getComputedStyle(headline).color:'',
    background:getComputedStyle(el).backgroundColor,
    actionForeground:action?getComputedStyle(action).color:'',
    actionBackground:action?getComputedStyle(action).backgroundColor:'',
    actionFontWeight:action?getComputedStyle(action).fontWeight:''
   };
 });
 assert(Boolean(ctaGeometry.copy&&ctaGeometry.action)&&ctaGeometry.copy[0]>=ctaGeometry.panel[0]-2&&ctaGeometry.copy[1]<=ctaGeometry.panel[1]+2&&ctaGeometry.action[0]>=ctaGeometry.panel[0]-2&&ctaGeometry.action[1]<=ctaGeometry.panel[1]+2,'track '+width+'px: invitation overflows panel '+JSON.stringify(ctaGeometry));
 assert(ctaGeometry.copy&&ctaGeometry.action&&(ctaGeometry.copy[3]<=ctaGeometry.action[2]+3||ctaGeometry.copy[1]<=ctaGeometry.action[0]+3),'track '+width+'px: invitation copy overlaps action '+JSON.stringify(ctaGeometry));
 assert(contrast(ctaGeometry.foreground,ctaGeometry.background)>=4.5,'track '+width+'px: invitation title contrast '+JSON.stringify(ctaGeometry));
 assert(contrast(ctaGeometry.actionForeground,ctaGeometry.actionBackground)>=4.5,'track '+width+'px: invitation action contrast '+JSON.stringify(ctaGeometry));
 assert(Number(ctaGeometry.actionFontWeight)%100===0,'track '+width+'px: invitation action font weight not standard '+JSON.stringify(ctaGeometry));
 await track.waitForTimeout(450);
