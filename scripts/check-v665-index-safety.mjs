...
// The original exact-version allowlist rejected newer releases even when the safety checks pass.
const versionParts=/^(\d+)\.(\d+)\.(\d+)$/.exec(String(packageJson.version||''));
const indexVersionOk=Boolean(versionParts) && (
  Number(versionParts[1])>0 || Number(versionParts[2])>66 ||
  (Number(versionParts[2])===66 && Number(versionParts[3])>=5)
);
must(indexVersionOk&&packageJson.scripts?.build?.includes('check-v665-index-safety.mjs'),'package version >=0.66.5 and build pipeline includes index safety check');
