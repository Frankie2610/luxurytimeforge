...
function ContrastAudit({settings}: {settings: Theme['settings']}) {
  const pairs = [
    ['Nền trang / chữ chính', settings.text, settings.background],
    ['Card / chữ chính', settings.text, settings.surface],
    ['Màu nhấn / chữ trên nền tối', settings.textOnDark, settings.accent],
    ['Nền trang / chữ phụ', settings.muted, settings.background],
  ] as const;
  return <section className="v50-contrast-audit" aria-label="Kiểm tra tương phản màu">
    <header><b>Kiểm tra tương phản WCAG</b><span>Chuẩn AA ≥ 4.5:1</span></header>
    {pairs.map(([label, foreground, background]) => {
      ...
      return <div className="v50-contrast-row" key={label}><span>{label}</span><span>{ratio === null ? '—' : `${ratio.toFixed(2)}:1`}</span><em className={passes ? '' : 'fail'}>{passes ? 'Đạt' : 'Cần sửa'}</em></div>;
    })}
  </section>;
}
...
function VirtualInspector({id, theme, patchTheme, extras, patchExtras}: {id: VirtualNode; theme: Theme; patchTheme: (theme: Theme) => void; extras: EditorExtras; patchExtras: (patch: Partial<EditorExtras>) => void}) {
  if (id === 'countdown') return <><InspectorHeader eyebrow="HEADER GROUP" title="Đếm ngược ưu đãi"/><div className="v19-inspector-fields"><SettingField name="Hiển thị" value={extras.showCountdown} onChange={(value) => {const enabled = Boolean(value); patchExtras({showCountdown: enabled}); if (enabled) patchTheme({...theme, settings: {...theme.settings, showAnnouncement: false}});}}/><SettingField name="Nội dung" value={extras.countdownText} onChange={(value) => patchExtras({countdownText: String(value)})}/><label className="v19-field"><span>Phối màu</span><div className="v19-schemes">{(['light', 'dark', 'green', 'red'] as const).map((scheme) => <button type="button" key={scheme} className={`${scheme} ${extras.countdownScheme === scheme ? 'active' : ''}`} onClick={() => patchExtras({countdownScheme: scheme})}>Aa</button>)}</div></label><p className="v19-note">Chỉ một thanh ưu đãi được hiển thị. Bật mục này sẽ tự tắt Thanh thông báo.</p></div></>;
  if (id === 'announcement') return <><InspectorHeader eyebrow="HEADER GROUP" title="Thanh thông báo"/><div className="v19-inspector-fields"><SettingField name="Hiển thị" value={theme.settings.showAnnouncement} onChange={(value) => {const enabled = Boolean(value); patchTheme({...theme, settings: {...theme.settings, showAnnouncement: enabled}}); if (enabled) patchExtras({showCountdown: false});}}/><SettingField name="Nội dung" value={theme.settings.announcement} onChange={(value) => patchTheme({...theme, settings: {...theme.settings, announcement: String(value)}})}/><p className="v19-note">Nội dung được rút gọn tự động trên điện thoại để luôn nằm gọn trong một dòng.</p></div></>;
...
function ThemeEditorV19({close}: {close: () => void}) {
  ...
  const themeVersions = themeState.versions || [];
  ...
  const [extras, setExtras] = useState<EditorExtras>(() => {
    const initial = readExtras();
    return draftTheme.settings.showAnnouncement && initial.showCountdown ? {...initial, showCountdown: false} : initial;
  });
  const [baselineExtras, setBaselineExtras] = useState<EditorExtras>(() => {
    const initial = readExtras();
    return draftTheme.settings.showAnnouncement && initial.showCountdown ? {...initial, showCountdown: false} : initial;
  });
  ...
  const treeSections = useMemo(() => {
    const query = treeQuery.trim().toLocaleLowerCase('vi');
    if (!query) return current.sections;
    const blockText = (blocks: ThemeBlock[]): string => blocks.map((block) => `${blockLabels[block.type]} ${Object.values(block.settings).join(' ')} ${blockText(block.children || [])}`).join(' ');
    return current.sections.filter((section) => `${sectionLabels[section.type]} ${Object.values(section.settings).join(' ')} ${blockText(section.blocks)}`.toLocaleLowerCase('vi').includes(query));
  }, [current.sections, treeQuery]);
  ...
  const saveDraftOnly = () => {saveThemeDraft(theme); saveThemeExtrasV23(extras); setBaseline(clone(theme)); setBaselineExtras(clone(extras)); toast.success('Đã lưu bản nháp');};
  const resetChanges = () => {
    setTheme(clone(baseline));
    setExtras(clone(baselineExtras));
    setHistory([]);
    setFuture([]);
  ...
  useEffect(() => {if (!dirty) return; const timer = window.setTimeout(() => {saveDraftRef.current(deferredTheme); saveThemeExtrasV23(deferredExtras);}, 1400); return () => window.clearTimeout(timer);}, [deferredTheme, deferredExtras, dirty]);
  ...
  useEffect(() => {iframeRef.current?.contentWindow?.postMessage({type: 'timeforge:editor-outlines', enabled: showOutlines}, window.location.origin);}, [showOutlines, previewSrc]);
  ...
  return <div className="tf-theme-editor-v499">
    <header className="v19-topbar">
      ...
      <div className="v19-top-actions v26-top-actions">
        ...
        <label className="v26-zoom-control" title="Thu phóng bản xem trước"><Maximize2/><select value={zoom} onChange={(event) => setZoom(Number(event.target.value))}><option value="56">56%</option><option value="68">68%</option><option value="84">84%</option><option value="100">100%</option><option value="115">115%</option></select></label>
        ...
        <DropdownMenu><DropdownMenuTrigger asChild><button type="button"><MoreHorizontal/></button></DropdownMenuTrigger><DropdownMenuContent>
          ...
          <DropdownMenuItem onSelect={() => setShowOutlines((value) => !value)}>{showOutlines ? <EyeOff/> : <Eye/>}{showOutlines ? 'Tắt khung section' : 'Hiện khung section'}</DropdownMenuItem>
          <DropdownMenuItem onSelect={saveDraftOnly}><Check/>Chỉ lưu bản nháp</DropdownMenuItem>
          <DropdownMenuItem onSelect={publish}><Sparkles/>Xuất bản theme</DropdownMenuItem>
          <DropdownMenuSeparator/>
          <DropdownMenuItem disabled={!dirty} onSelect={resetChanges}><RotateCcw/>Hủy thay đổi chưa lưu</DropdownMenuItem>
          ...
          <DropdownMenuItem onSelect={() => {restoreThemeVersion(themeVersions[0]?.id || ''); if (themeVersions[0]) {setTheme(clone(themeVersions[0].theme)); setBaseline(clone(themeVersions[0].theme));}}}><History/>Khôi phục bản gần nhất</DropdownMenuItem>
      ...
      <aside className={`v19-sidebar ${sidebarOpen ? 'open' : ''}`}>
        {mode === 'sections' && <>
          <section className="v19-sidebar-tree">
            <header className="v19-template-head"><div><h2>{templateLabels[template]}</h2><small>{current.sections.length} section · {dirty ? 'Có thay đổi chưa lưu' : 'Đã đồng bộ'}</small></div><button type="button" onClick={() => setPicker(true)}><ChevronDown/></button></header><div className="v23-storefront-link-state"><Check/><span><b>Liên kết storefront đang hoạt động</b><small>Section, block và cài đặt được áp dụng vào đúng template sau khi bấm Lưu.</small></span></div>
            ...
            <div className="v507-tree-tools">
              <label><Search/><input value={treeQuery} onChange={(event) => setTreeQuery(event.target.value)} placeholder="Tìm section hoặc block"/>{treeQuery && <button type="button" onClick={() => setTreeQuery('')} aria-label="Xóa tìm kiếm"><X/></button>}</label>
              <div><button type="button" onClick={() => setExpanded(Object.fromEntries(current.sections.map((section) => [section.id, true])))}><ChevronDown/>Mở hết</button><button type="button" onClick={() => setExpanded(Object.fromEntries(current.sections.map((section) => [section.id, false])))}><ChevronRight/>Thu gọn</button></div>
            </div>
            <div className="v19-tree-scroll">
              {!treeQuery && <section className="v19-tree-group"><h3>Header group</h3><VirtualRow icon={<Sparkles/>} label="Đếm ngược ưu đãi" selected={selection.kind === 'virtual' && selection.id === 'countdown'} visible={extras.showCountdown} onSelect={() => setSelection({kind: 'virtual', id: 'countdown'})} onToggle={() => {const enabled = !extras.showCountdown; patchExtras({showCountdown: enabled}); if (enabled) commit({...theme, settings: {...theme.settings, showAnnouncement: false}});}}/><VirtualRow icon={<LayoutPanelLeft/>} label="Thanh thông báo" selected={selection.kind === 'virtual' && selection.id === 'announcement'} visible={theme.settings.showAnnouncement} onSelect={() => setSelection({kind: 'virtual', id: 'announcement'})} onToggle={() => {const enabled = !theme.settings.showAnnouncement; commit({...theme, settings: {...theme.settings, showAnnouncement: enabled}}); if (enabled) patchExtras({showCountdown: false});}}/><VirtualRow icon={<Menu/>} label="Header" selected={selection.kind === 'virtual' && selection.id === 'header'} onSelect={() => setSelection({kind: 'virtual', id: 'header'})}/></section>}
              <section className="v19-tree-group template-group"><h3>{treeQuery ? `Kết quả · ${treeSections.length}` : 'Template'}</h3><DndContext sensors={sensors} onDragEnd={sectionDrag}><SortableContext items={treeSections.map((section) => section.id)} strategy={verticalListSortingStrategy}>{treeSections.map((section) => <SortableSectionRow key={section.id} section={section} expanded={treeQuery ? true : (expanded[section.id] ?? true)} selected={selection.kind !== 'virtual' && selection.sectionId === section.id && selection.kind === 'section'} onExpand={() => setExpanded((items) => ({...items, [section.id]: !(items[section.id] ?? true)}))} onSelect={() => setSelection({kind: 'section', sectionId: section.id})} onToggle={() => patchSection(section.id, {visible: !section.visible})} onDuplicate={() => duplicateSection(section)} onDelete={() => deleteSection(section)}><DndContext sensors={sensors} onDragEnd={(event) => blockDrag(section.id, event)}><SortableContext items={section.blocks.map((block) => block.id)} strategy={verticalListSortingStrategy}><div className="v19-block-list">{section.blocks.map((block) => <SortableBlockRow key={block.id} block={block} selectedId={selection.kind === 'block' ? selection.blockId : ''} onSelect={(blockId) => setSelection({kind: 'block', sectionId: section.id, blockId})} onToggle={(blockId) => {const item = findBlock(section.blocks, blockId); if (item) patchBlock(section.id, blockId, {visible: !item.visible});}} onDuplicate={(blockId) => duplicateBlock(section.id, blockId)} onDelete={(blockId) => deleteBlock(section.id, blockId)}/>)}</div></SortableContext></DndContext></SortableSectionRow>)}</SortableContext></DndContext>{treeQuery && !treeSections.length && <div className="v507-tree-empty"><Search/><b>Không tìm thấy nội dung</b><span>Thử tên section, block hoặc nội dung khác.</span></div>}<div className="v19-add-section"><button type="button" onClick={() => setSectionLibraryOpen(true)}><Plus/>Thêm section</button></div></section>
