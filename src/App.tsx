...
function RouteLoading(){return <div className="route-loading" aria-label="Đang tải trang" aria-busy="true"><div className="route-loading-bar"/><div className="route-loading-brand"><img src="/luxury-timeforge-logo.svg" alt="" aria-hidden="true"/><i/><b>Đang tải Luxury Timeforge</b></div></div>}

export function App(){return <Suspense fallback={<RouteLoading/>}><Routes>
 <Route element={<StoreLayout/>}>
  <Route path="/" element={<Home/>}/><Route path="/collections" element={<CollectionPage/>}/><Route path="/collections/:handle" element={<CollectionPage/>}/><Route path="/products/:handle" element={<ProductPage/>}/><Route path="/search" element={<SearchPage/>}/><Route path="/cart" element={<CartPageV11/>}/><Route path="/checkout" element={<CheckoutPageV11/>}/><Route path="/order-confirmation/:id" element={<OrderConfirmationV11/>}/><Route path="/blogs" element={<BlogIndexV18/>}/><Route path="/blogs/:handle" element={<BlogPostPageV18/>}/><Route path="/pages/:slug" element={<ContentPage/>}/><Route path="/404" element={<NotFound/>}/>
 </Route>
