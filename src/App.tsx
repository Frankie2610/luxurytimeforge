...
const StoreLayout=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.StoreLayoutV10})));
const Home=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.HomeV10})));
const CollectionPage=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.CollectionPageV10})));
const ProductPage=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.ProductPageV10})));
const SearchPage=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.SearchPageV10})));
const WishlistPageV53=lazy(()=>import('./wishlist-page-v53').then(m=>({default:m.WishlistPageV53})));
const ComparePageV57=lazy(()=>import('./storefront-tools-v57').then(m=>({default:m.ComparePageV57})));
const WatchFinderPageV57=lazy(()=>import('./storefront-tools-v57').then(m=>({default:m.WatchFinderPageV57})));
const ContentPage=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.ContentPageV10})));
const NotFound=lazy(()=>import('./storefront-route-v685').then(m=>({default:m.NotFoundV10})));
const CartPageV11=lazy(()=>import('./checkout-route-v685').then(m=>({default:m.CartPageV11})));
...
const MetaAdsV57=lazy(()=>import('./meta-ads-v57').then(m=>({default:m.MetaAdsV57})));
...
const ReviewsAdminV60=lazy(()=>import('./reviews-admin-v60').then(m=>({default:m.ReviewsAdminV60})));
const SeoHeadV60=lazy(()=>import('./seo-head-v60').then(m=>({default:m.SeoHeadV60})));
...
const ProductGroupsAdminV504=lazy(()=>import('./product-groups-admin-v504').then(m=>({default:m.ProductGroupsAdminV504})));
const SeoLandingPageV657=lazy(()=>import('./seo-landing-v657').then(m=>({default:m.SeoLandingPageV657})));
const ReferralLandingV68=lazy(()=>import('./referral-landing-v68').then(m=>({default:m.ReferralLandingV68})));
const ReferralAdminV68=lazy(()=>import('./referral-admin-v68').then(m=>({default:m.ReferralAdminV68})));
const ProductOpsCenterV681=lazy(()=>import('./product-ops-v681').then(m=>({default:m.ProductOpsCenterV681})));
...
export function App(){return <><StoreIdentityHeadSync/><Suspense fallback={null}><SeoHeadV60/></Suspense><Suspense fallback={<RouteLoading/>}><Routes>
 <Route element={<StoreLayout/>}>
  <Route path="/" element={<Home/>}/><Route path="/dong-ho-nam" element={<SeoLandingPageV657/>}/><Route path="/dong-ho-nu" element={<SeoLandingPageV657/>}/><Route path="/dong-ho-duoi-5-trieu" element={<SeoLandingPageV657/>}/><Route path="/dong-ho-sale" element={<SeoLandingPageV657/>}/><Route path="/collections" element={<CollectionPage/>}/><Route path="/collections/:handle" element={<CollectionPage/>}/><Route path="/products/:handle" element={<ProductPage/>}/><Route path="/search" element={<SearchPage/>}/><Route path="/wishlist" element={<WishlistPageV53/>}/><Route path="/compare" element={<ComparePageV57/>}/><Route path="/watch-finder" element={<WatchFinderPageV57/>}/><Route path="/cart" element={<CartPageV11/>}/><Route path="/checkout" element={<CheckoutPageV11/>}/><Route path="/payment/payos/return" element={<PayOSReturnPageV4927/>}/><Route path="/order-confirmation/:id" element={<OrderConfirmationV11/>}/><Route path="/blogs" element={<BlogIndexV18/>}/><Route path="/blogs/:handle" element={<BlogPostPageV18/>}/><Route path="/pages/:slug" element={<ContentPage/>}/><Route path="/404" element={<NotFound/>}/>
 </Route>
 <Route path="/ref/:code" element={<ReferralLandingV68/>}/><Route path="/account/login" element={<CustomerLoginV12/>}/><Route path="/account" element={<CustomerAccountV12/>}/><Route path="/account/orders/:id" element={<CustomerOrderV12/>}/><Route path="/account/orders/:id/return" element={<CustomerReturnV13/>}/><Route path="/track-order" element={<TrackOrderV12/>}/>
 ...
 <Route path="/admin" element={<ProtectedAdmin><PermissionGate><AdminLayout/></PermissionGate></ProtectedAdmin>}>
  <Route index element={<DashboardV3/>}/><Route path="orders" element={<OrdersV11/>}/><Route path="orders/:id" element={<OrderDetailV12/>}/><Route path="returns" element={<ReturnsAdminV13/>}/><Route path="draft-orders" element={<DraftOrdersV12/>}/><Route path="draft-orders/new" element={<DraftOrderEditorV12/>}/><Route path="draft-orders/:id" element={<DraftOrderEditorV12/>}/><Route path="products" element={<ProductsV9/>}/><Route path="products/new" element={<ProductEditor/>}/><Route path="products/:id" element={<ProductEditor/>}/><Route path="collections" element={<Collections/>}/><Route path="product-groups" element={<ProductGroupsAdminV504/>}/><Route path="inventory" element={<InventoryV3/>}/><Route path="customers" element={<CustomersV3/>}/><Route path="customer-segments" element={<CustomerSegmentsV11/>}/><Route path="analytics" element={<AnalyticsV15/>}/><Route path="marketing/meta" element={<MetaAdsV57/>}/><Route path="discounts" element={<Discounts/>}/><Route path="referrals" element={<ReferralAdminV68/>}/><Route path="product-ops" element={<ProductOpsCenterV681/>}/><Route path="activity" element={<ActivityLog/>}/><Route path="blogs" element={<AdminBlogsV18/>}/><Route path="reviews" element={<ReviewsAdminV60/>}/><Route path="pages" element={<ContentPagesAdminV23/>}/><Route path="import-export" element={<ImportExport/>}/><Route path="online-store" element={<OnlineStoreV19/>}/><Route path="settings" element={<SettingsPage/>}/><Route path="settings/integrations" element={<IntegrationsV13/>}/><Route path="settings/team" element={<TeamPermissionsV20/>}/>
 </Route>
