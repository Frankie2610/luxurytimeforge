...
export async function uploadCloudinaryImage(file:File,subfolder='shop'):Promise<CloudinaryUploadResult>{
  ...
  if(file.size>8*1024*1024)throw new Error('Logo phải nhỏ hơn 8 MB.');

  const data=new FormData();
  data.append('file',file);
  data.append('upload_preset',uploadPreset);
  if(baseFolder)data.append('folder',`${baseFolder}/${subfolder}`);
  data.append('context','source=timeforge-admin|asset=shop-logo');
  ...
  if(!response.ok||payload.error)throw new Error(payload.error?.message||'Không thể tải logo lên Cloudinary.');
  const url=String(payload.secure_url||payload.url||'');
  if(!url)throw new Error('Cloudinary không trả về đường dẫn logo.');
