 ...
 return <details className={`tf704-rules ${compact?'is-compact':''}`} id={compact?undefined:'the-le-thanh-vien'}>
  ...
  <div className="tf704-rules-content">
   ...
   <div className="tf704-tier-table" role="table" aria-label="Điều kiện và quyền lợi từng hạng">
    ...
    {settings.tiers.map(tier=><div className="tf704-tier-row" role="row" key={tier.id}><strong role="cell">{tier.label}</strong><span role="cell">{tier.minSpend?money(tier.minSpend):'Đăng ký miễn phí'}</span><span role="cell">×{tier.creditMultiplier.toLocaleString('vi-VN',{maximumFractionDigits:2})}</span><span role="cell">{settings.personalHoldEnabled?`${tier.holdMinutes} phút`:'Tạm chưa áp dụng'}</span></div>)}
   </div>
   <p>Cứ mỗi {money(settings.creditSpendUnit)} chi tiêu hợp lệ, bạn nhận 1 điểm cơ bản; hệ số tính theo hạng tại thời điểm mua. Điểm được ghi nhận sau khi thanh toán thành công và có thể được điều chỉnh khi hoàn trả.</p>
...
export function CustomerAccountV12() {
  ...
  return <AccountShell customer={customer}>
    ...
    <section className="v12-account-hero v524-account-hero tf690-member-hero">
      <div className="v524-account-hero-copy"><small>MY TIMEFORGE</small><h1>Xin chào, {String(customer.name||'bạn').trim().split(' ').slice(-1)[0]}.</h1><p>Không chỉ xem đơn hàng: đây là nơi theo dõi bộ sưu tập, quyền lợi thành viên và những mẫu đáng cân nhắc tiếp theo.</p><div className="tf582-account-pills" aria-label="Tiện ích tài khoản"><span>Vault sở hữu</span><span>Forge Credits</span><span>Decision Radar</span><span>Referral</span></div><div className="v524-account-quick-actions"><Link to="/collections"><ShoppingBag/>Khám phá đồng hồ</Link><Link to="/wishlist"><Heart/>Mẫu đang lưu</Link></div></div>
...
export function TrackOrderV12() {
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();setBusy(true);setError('');setSearched(false);try{const response=await fetch('/api/account/request-otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'track-order',number,identity})});const data=await response.json().catch(()=>({})) as{order?:Order;message?:string};if(!response.ok||!data.order)throw new Error(data.message||'Không tìm thấy đơn hàng phù hợp.');setFound(normalizeCustomerOrder(data.order))}catch(reason){setFound(null);setError(reason instanceof Error?reason.message:'Không thể tra cứu đơn hàng.')}finally{setSearched(true);setBusy(false)}};
  const workflow=readWorkflow();const fulfillment=found?workflow.fulfillments.find(item=>item.orderId===found.id):undefined;return <AccountShell><section className="v12-track-page tf710-track-page"><div className="v12-track-copy tf710-track-hero"><small><PackageSearch/> THEO DÕI ĐƠN HÀNG</small><h1>Đơn hàng của bạn đang ở đâu?</h1><p>Nhập mã đơn và thông tin liên hệ đã dùng khi mua để xem trạng thái giao hàng cùng các sản phẩm trong đơn. Không cần đăng ký thành viên.</p></div><form className="tf710-track-form" onSubmit={submit}><label><span>Mã đơn hàng</span><input value={number} onChange={event=>setNumber(event.target.value)} placeholder="TF-260919-1234"/></label><label><span>Email hoặc số điện thoại</span><input value={identity} onChange={event=>setIdentity(event.target.value)} placeholder="Thông tin dùng khi đặt hàng"/></label><button className="v12-primary" disabled={busy||!number.trim()||!identity.trim()}><Search/>{busy?'Đang tra cứu...':'Tra cứu đơn hàng'}</button></form><AnimatePresence mode="wait">{searched&&(found?<motion.section key="found" className="v12-track-result tf710-track-result" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}><header><div><small>{found.number}</small><h2>{statusLabel[found.status]}</h2></div><span>{fmt(found.createdAt)}</span></header><Progress order={found}/><div><PackageCheck/><span><b>{fulfillmentLabel[found.fulfillmentStatus]}</b><small>{found.trackingNumber?`${found.shippingCarrier||'Vận chuyển'} · ${found.trackingNumber}`:fulfillment?.trackingNumber?`${fulfillment.carrier} · ${fulfillment.trackingNumber}`:'Chưa có mã vận đơn'}</small></span></div><div className="tf710-order-items">
 <div className="tf710-order-heading"><h3>Sản phẩm trong đơn</h3><span>{found.lines.reduce((sum,line)=>sum+line.quantity,0)} sản phẩm</span></div>
 {found.lines.length?found.lines.map(line=><article className="tf710-order-item" key={line.id}>
  <div className="tf710-item-photo"><SmartImage src={line.image} alt={line.title} width={160} height={160}/></div>
  <div className="tf710-item-copy"><b>{line.title}</b>
   {(line.variantTitle||line.sku)&&<small>{[line.variantTitle,line.sku&&`SKU: ${line.sku}`].filter(Boolean).join(' · ')}</small>}
   <span>Số lượng: {line.quantity} · {money(line.unitPrice)}/chiếc</span>
  </div><strong>{money(line.lineTotal)}</strong></article>):<p>Danh sách sản phẩm đang được cập nhật. Liên hệ cửa hàng nếu cần hỗ trợ.</p>}
 </div>
 <div className="tf710-order-summary"><span>Người nhận: <b>{found.customerName||'Khách hàng'}</b></span>
 <span>Thanh toán: <b>{found.paymentStatus==='paid'?'Đã thanh toán':found.paymentStatus==='refunded'?'Đã hoàn tiền':'Chờ thanh toán / COD'}</b></span>
 <span className="tf710-grand-total">Tổng tiền đơn hàng <strong>{money(found.total)}</strong></span></div></motion.section>:<motion.div key="empty" className="v12-form-error" initial={{opacity:0}} animate={{opacity:1}}>{error||'Không tìm thấy đơn hàng phù hợp. Kiểm tra lại mã đơn và thông tin liên hệ.'}</motion.div>)}</AnimatePresence><Link className="v12-track-account" to="/account/login"><UserRound/>Khám phá TimeForge Member và những quyền lợi dành riêng cho bạn</Link></section></AccountShell>;
}
