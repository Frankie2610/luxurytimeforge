...
function MemberRules({settings,compact=false}:{settings:MemberSettings;compact?:boolean}){
 return <details className={`tf704-rules ${compact?'is-compact':''}`} id={compact?undefined:'the-le-thanh-vien'}>
  ...
  <div className="tf704-rules-content">
   ...
   <div className="tf704-tier-perks" aria-label="Quyền lợi theo từng hạng">{settings.tiers.map(tier=><article key={tier.id}><strong>{tier.label}</strong><p>{tier.benefits.length?tier.benefits.join(' · '):'Quyền lợi được cập nhật theo từng chương trình.'}</p></article>)}</div>
