...
const K={p:'tf.react.products',c:'tf.react.collections',groups:'tf.react.product-groups',u:'tf.react.customers',cart:'tf.react.cart',theme:'tf.react.theme.v2',themeLegacy:'tf.react.theme',profile:'tf.react.store-profile',headers:'tf.react.headers',orders:'tf.react.orders',discounts:'tf.react.discounts',adjustments:'tf.react.inventory-adjustments',activities:'tf.react.activities',newsletter:'tf.react.newsletter-subscribers',reviews:'tf.react.store-reviews'};
...
type V={
 products:Product[];collections:Collection[];productGroups:ProductGroup[];customers:Customer[];newsletterSubscribers:NewsletterSubscriber[];reviews:StoreReview[];orders:Order[];discounts:Discount[];adjustments:InventoryAdjustment[];activities:Activity[];
 ...
 subscribeNewsletter:(email:string,source?:string)=>Promise<'created'|'exists'|'reactivated'|'invalid'>;updateNewsletterSubscriber:(id:string,patch:Partial<NewsletterSubscriber>)=>void;deleteNewsletterSubscriber:(id:string)=>void;saveReview:(review:StoreReview)=>Promise<void>;deleteReview:(id:string)=>void;
...
type NewsletterActions={subscribeNewsletter:(email:string,source?:string)=>Promise<'created'|'exists'|'reactivated'|'invalid'>};
const NewsletterActionsC=createContext<NewsletterActions|null>(null);
...
export function CommerceProvider({children}:{children:ReactNode}){
 ...
 const[reviews,setReviews]=useState<StoreReview[]>(()=>asList<StoreReview>(load(K.reviews,[])));
 ...
 useIdleLocalStorage(K.reviews,reviews);
 ...
 const subscribeNewsletter=async(rawEmail:string,source='footer')=>{const email=rawEmail.trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return'invalid' as const;const existing=newsletterSubscribers.find(item=>item.email===email);const now=new Date().toISOString();if(existing?.status==='active')return'exists' as const;const created:NewsletterSubscriber={id:existing?.id||newsletterKey(email),email,source,status:'active',createdAt:existing?.createdAt||now,updatedAt:now};if(firebaseClient.enabled)await firebaseClient.write(`timeforge/newsletterSubscribers/${created.id}`,created);const next:NewsletterSubscriber[]=existing?newsletterSubscribers.map(item=>item.id===existing.id?created:item):[created,...newsletterSubscribers];setNewsletterSubscribers(next);const matching=customers.find(item=>item.email.toLowerCase()===email);if(matching&&!matching.acceptsMarketing){const nextCustomers=customers.map(item=>item.id===matching.id?{...item,acceptsMarketing:true}:item);setCustomers(nextCustomers);sync('timeforge/customers',nextCustomers)}log('customer',existing?.id||email,existing?'Kích hoạt lại email marketing':'Đăng ký email marketing',email);return existing?'reactivated' as const:'created' as const};
 ...
 const updateOrder=(id:string,patch:Partial<Order>)=>setOrders(cur=>{
  ...
  if(user?.access==='active'){
   if(firebaseClient.enabled)void firebaseClient.write(`timeforge/orders/${id}`,updated).then(()=>{if(found.paymentStatus!=='paid'&&updated.paymentStatus==='paid')return fetch('/api/price-alerts/process',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'warranty',orderId:id})}).catch(()=>null)}).catch(reportFirebaseError);
