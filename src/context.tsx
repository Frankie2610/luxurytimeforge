...
import type{Activity,CartLine,CheckoutPayload,Collection,Customer,Discount,DiscountEvaluation,InventoryAdjustment,NewsletterSubscriber,Order,OrderStatus,Product,ProductGroup,Theme,ThemeState,ThemeVersion}from'./types';
...
const K={p:'tf.react.products',c:'tf.react.collections',groups:'tf.react.product-groups',u:'tf.react.customers',cart:'tf.react.cart',theme:'tf.react.theme.v2',themeLegacy:'tf.react.theme',headers:'tf.react.headers',orders:'tf.react.orders',discounts:'tf.react.discounts',adjustments:'tf.react.inventory-adjustments',activities:'tf.react.activities',newsletter:'tf.react.newsletter-subscribers'};
...
export function CommerceProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>localStorage.setItem(K.groups,JSON.stringify(productGroups)),[productGroups]);
 ...
 useEffect(()=>{
  ...
  ]).then(([productResult,collectionResult,groupResult,discountResult,publishedResult])=>{
   if(!active)return;
