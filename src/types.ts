...
export interface Condition{field:'vendor'|'productType'|'tag'|'status'|'price'|'inventory';operator:'equals'|'contains'|'greater_than'|'less_than';value:string}
export interface Collection{id:string;handle:string;title:string;description:string;type:'manual'|'automatic';status:'active'|'draft';image:string;productIds:string[];conditions:Condition[]}
...
export interface Order{id:string;number:string;createdAt:string;updatedAt:string;customerId:string;customerName:string;customerEmail:string;customerPhone:string;shippingAddress:ShippingAddress;lines:OrderLine[];subtotal:number;discountCode:string;discountAmount:number;shippingAmount:number;taxAmount:number;total:number;currency:'VND';status:OrderStatus;paymentStatus:PaymentStatus;fulfillmentStatus:FulfillmentStatus;paymentMethod:PaymentMethod;paymentProvider?:'payos'|'generic';paymentReference?:string;paymentOrderCode?:number;paidAt?:string;note:string;source:'storefront'|'admin'}
...
export interface IntegrationSettings{payment:{cod:boolean;bankTransfer:boolean;online:boolean;onlineProvider:'generic'|'payos';createEndpoint:string;bankName:string;bankAccountName:string;bankAccountNumber:string};shipping:{defaultCarrier:string;trackingUrlTemplate:string;insured:boolean;freeShippingThreshold:number};customerAccount:{sessionMinutes:number;requireOrderChallenge:boolean}}
