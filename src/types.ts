...
export interface CustomerMembership{status:'active'|'inactive';joinedAt:string;source:'self_register'|'admin'|'migration';}
export interface Customer{id:string;name:string;email:string;phone:string;ordersCount:number;totalSpent:number;tags:string[];createdAt:string;birthDate?:string;membership?:CustomerMembership;addresses?:CustomerAddress[];notes?:CustomerNote[];acceptsMarketing?:boolean}
...
export interface Order{id:string;number:string;createdAt:string;updatedAt:string;customerId:string;customerName:string;customerEmail:string;customerPhone:string;shippingAddress:ShippingAddress;lines:OrderLine[];subtotal:number;discountCode:string;discountAmount:number;promotionDiscountAmount?:number;paymentDiscountAmount?:number;paymentDiscountLabel?:string;referralCode?:string;referralDiscountAmount?:number;referralReferrerId?:string;referralFraudScore?:number;referralRiskLevel?:'low'|'medium'|'high';referralRewardStatus?:'pending'|'review'|'qualified'|'blocked'|'revoked';referralRiskSignals?:string[];referralQualifiedAt?:string;shippingAmount:number;taxAmount:number;total:number;currency:'VND';status:OrderStatus;paymentStatus:PaymentStatus;fulfillmentStatus:FulfillmentStatus;paymentMethod:PaymentMethod;paymentProvider?:'payos'|'generic';paymentReference?:string;paymentOrderCode?:number;paymentConfirmationSource?:PaymentConfirmationSource;paidAt?:string;bankTransferConfirmedAt?:string;bankAccountId?:string;bankName?:string;bankAccountName?:string;bankAccountNumber?:string;bankTransferContent?:string;shippingCarrier?:string;trackingNumber?:string;trackingUrl?:string;shippedAt?:string;deliveredAt?:string;warrantyAutomationStatus?:'ready'|'pending'|'failed';warrantyRecordCount?:number;warrantySyncedAt?:string;warrantyActivationEmailStatus?:'sent'|'failed'|'not_configured';note:string;source:'storefront'|'admin';memberOpportunityId?:string;memberCreditMultiplier?:number;memberBonusLabel?:string}
export interface CheckoutPayload{customer:{name:string;email:string;phone:string};shippingAddress:ShippingAddress;paymentMethod:PaymentMethod;note:string;discountCode:string;referralCode?:string;referralDeviceId?:string;referralEmailVerified?:boolean}
...
export interface StoreProfile{storeName:string;storeDescription:string;seoTitle:string;seoDescription:string;storePhone:string;storeEmail:string;storeAddress:string;taxId:string;facebookUrl:string;instagramUrl:string;tiktokUrl:string;recruitmentUrl:string;logoImage:string;socialShareImage:string;updatedAt:string}
