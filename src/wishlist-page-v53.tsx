...
import './v576-wishlist-readability.css';
...
export function WishlistPageV53() {
  ...
  const toggleCompare = (productId: string) => {
    ...
    toast.success(result === 'added' ? 'Đã thêm vào bảng so sánh' : 'Đã xóa khỏi bảng so sánh');
  };
    ...
    {wishlistProducts.length ? <>
      ...
      <section className="tf53-wishlist-grid" aria-label="Sản phẩm yêu thích">
        {visibleProducts.map((product) => {
          ...
          const selectedForCompare = compare.includes(product.id);
