...
import {MemberLoyaltyAdminV69} from './member-admin-v69';
...
function IndexSearch({value, onChange, placeholder}: {value: string; onChange: (value: string) => void; placeholder: string}) {
  return <label className="tf4925-admin-search"><Search /><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /></label>;
}
...
function DiscountModal({discount, close}: {discount: Discount | null; close: () => void}) {
  ...
  return <div className="tf4923-discount-modal-shell" onClick={close}><form className="tf4923-discount-modal" onClick={(event)=>event.stopPropagation()} onSubmit={handleSubmit(submit)}>
    <header><div><span><Tag/>THIẾT LẬP ƯU ĐÃI</span><h2>{discount?.code||'Tạo mã giảm giá'}</h2><p>Thiết lập điều kiện, thời gian và giới hạn áp dụng.</p></div><button type="button" onClick={close} aria-label="Đóng"><X/></button></header>
    <div className="tf4923-discount-form-grid">
      <label><span>Mã giảm giá</span><input {...register('code')} onInput={(event)=>{event.currentTarget.value=event.currentTarget.value.toUpperCase().replace(/\s/g,'')}} placeholder="SUMMER20"/>{errors.code&&<small>{errors.code.message}</small>}</label>
      <label><span>Tên nội bộ</span><input {...register('title')} placeholder="Ưu đãi mùa hè"/>{errors.title&&<small>{errors.title.message}</small>}</label>
      <label><span>Loại ưu đãi</span><select {...register('type')}><option value="percentage">Phần trăm</option><option value="fixed_amount">Số tiền cố định</option><option value="free_shipping">Miễn phí vận chuyển</option></select></label>
      {type!=='free_shipping'&&<label><span>Giá trị {type==='percentage'?'(%)':'(₫)'}</span><input type="number" {...register('value')}/></label>}
      <label><span>Đơn hàng tối thiểu</span><input type="number" {...register('minimumSubtotal')}/></label>
      <label><span>Giới hạn lượt dùng</span><input type="number" {...register('usageLimit')}/><small>Nhập 0 nếu không giới hạn.</small></label>
      <label><span>Bắt đầu</span><input type="datetime-local" {...register('startsAt')}/></label>
      <label><span>Kết thúc</span><input type="datetime-local" {...register('endsAt')}/></label>
    </div>
    <label className="tf4923-discount-toggle"><span><b>Kích hoạt mã giảm giá</b><small>Khách có thể áp dụng mã trong thời gian hiệu lực.</small></span><input type="checkbox" {...register('active')}/></label>
    <footer>{existing&&<button type="button" className="is-danger" onClick={()=>{if(discount&&confirm('Xóa mã giảm giá này?')){deleteDiscount(discount.id);close()}}}><Trash2/>Xóa mã</button>}<span/><button type="button" onClick={close}>Hủy</button><button className="is-primary">Lưu mã giảm giá</button></footer>
  </form></div>;
}
...
export function DiscountsV10() {
  ...
  const activeCount=discounts.filter(item=>item.active&&new Date(item.endsAt).getTime()>=Date.now()).length;
  const usedCount=discounts.reduce((sum,item)=>sum+item.usageCount,0);
  const views=[{id:'all',label:'Tất cả',count:discounts.length},{id:'active',label:'Đang hoạt động',count:discounts.filter(item=>item.active).length},{id:'inactive',label:'Đã tắt',count:discounts.filter(item=>!item.active).length},{id:'expired',label:'Hết hạn',count:discounts.filter(item=>new Date(item.endsAt).getTime()<Date.now()).length}];
  return <div className="tf4923-discounts">
    <section className="tf4923-discount-hero"><div className="tf4923-discount-hero-copy"><span><Tag/>TRUNG TÂM ƯU ĐÃI</span><h2>Mã giảm giá rõ điều kiện, dễ kiểm soát</h2><p>Theo dõi hiệu lực, lượt sử dụng và phạm vi áp dụng mà không bị trộn với các lớp giao diện quản trị cũ.</p></div><div className="tf4923-discount-metrics"><article><b>{activeCount}</b><span>Đang hiệu lực</span></article><article><b>{usedCount}</b><span>Lượt đã dùng</span></article><button onClick={()=>setEditing(null)}><Plus/>Tạo mã giảm giá</button></div></section>
    <section className="tf4923-discount-surface">
      <header className="tf4923-discount-toolbar"><nav>{views.map(item=><button key={item.id} className={view===item.id?'is-active':''} onClick={()=>setView(item.id)}>{item.label}<span>{item.count}</span></button>)}</nav><label><Search/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Tìm mã hoặc tên ưu đãi"/></label></header>
      <div className="tf4923-discount-list">{filtered.map(item=>{const expired=new Date(item.endsAt).getTime()<Date.now();return <article key={item.id}><div className="tf4923-discount-icon"><Tag/></div><div className="tf4923-discount-name"><button onClick={()=>setEditing(item)}>{item.code}</button><p>{item.title}</p><small>{item.type==='percentage'?`${item.value}%`:item.type==='fixed_amount'?money(item.value):'Miễn phí vận chuyển'} · Đơn từ {money(item.minimumSubtotal)}</small></div><div className="tf4923-discount-usage"><b>{item.usageCount}{item.usageLimit?` / ${item.usageLimit}`:''}</b><small>Lượt sử dụng</small></div><span className={`tf4923-discount-state ${expired?'is-expired':item.active?'is-active':'is-off'}`}>{expired?'Hết hạn':item.active?'Đang hoạt động':'Đã tắt'}</span><button className="tf4923-discount-more" onClick={()=>setEditing(item)} aria-label={`Chỉnh sửa ${item.code}`}><MoreHorizontal/></button></article>})}{!filtered.length&&<div className="tf4923-discount-empty"><Tag/><h3>Không có mã giảm giá</h3><p>Tạo mã mới hoặc thay đổi chế độ xem hiện tại.</p><button onClick={()=>setEditing(null)}><Plus/>Tạo mã đầu tiên</button></div>}</div>
    </section>
    {editing!==undefined&&<DiscountModal discount={editing} close={()=>setEditing(undefined)}/>} 
  </div>;
}
