...
export const createSection = (type: SectionType): Section => {
  switch (type) {
    ...
    case 'collectionGrid': return s(type, {columns: 4, showFilter: true, showSort: true, showCount: true, pageSize: 24});
...
function normalizeThemeV27(theme: Theme): Theme {
  ...
  const normalizeBlocksV41 = (blocks: ThemeBlock[]): ThemeBlock[] => blocks.map((block) => ({
    ...
    settings: block.type === 'buyButtons'
      ...
      : {...(block.settings || {})},
    children: block.children?.length ? normalizeBlocksV41(block.children) : block.children,
  }));
