...
const blankReview=():StoreReview=>({
  id:'',customerName:'',title:'',text:'',image:'',rating:5,source:'Khách hàng TimeForge',status:'published',featured:true,sortOrder:0,
...
export function ReviewsAdminV60(){
  ...
  const remove=(item:StoreReview)=>{
    ...
    deleteReview(item.id);toast.success('Đã xóa testimonial.');
  };
  ...
  return <div className="tf60-reviews-admin">
    <section className="tf60-review-hero">
      <div><small>SOCIAL PROOF</small><h2>Review & testimonial</h2><p>Quản lý lời nhận xét dạng chữ hoặc ảnh để hiển thị tự động ở khu vực “Trải nghiệm từ khách hàng” trên storefront.</p></div>
    ...
    <section className="tf60-review-list">
      <header><div><h3>Nội dung đang quản lý</h3><p>Review “Xuất bản” sẽ được ưu tiên hiển thị; đánh dấu “Nổi bật” để đưa lên trước.</p></div><span>{publishedCount} đang hiển thị</span></header>
      {sorted.length?<div className="tf60-review-grid">{sorted.map(item=><article key={item.id} className={item.status==='draft'?'is-draft':''}>
        ...
        <div className="tf60-review-card-copy">
          <div className="tf60-review-badges"><span className={item.status==='published'?'is-live':''}>{item.status==='published'?'Đang hiển thị':'Bản nháp'}</span>{item.featured&&<span>Nổi bật</span>}</div>
