...
export function OrdersV11() {
  ...
  return <AdminResourceFrame className="s11-page"><AdminResourceIntro eyebrow="Vận hành" title="Đơn hàng" description="Theo dõi toàn bộ vòng đời từ thanh toán đến giao hàng, hoàn trả và hoàn tiền." actions={<Link className="s11-primary" to="/admin/draft-orders/new"><Plus />Tạo đơn nháp</Link>}/>
    <AdminResourceSurface className="s11-index-card"><div className="s11-view-tabs">{([['all', 'Tất cả'], ['open', 'Đang mở'], ['confirmed', 'Đã xác nhận'], ['completed', 'Hoàn tất'], ['cancelled', 'Đã hủy']] as const).map(([id, label]) => <button key={id} className={view === id ? 'active' : ''} onClick={() => setView(id)}>{label}<span>{id === 'all' ? orders.length : orders.filter((item) => item.status === id).length}</span></button>)}</div>
